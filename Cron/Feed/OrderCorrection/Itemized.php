...
class Itemized extends OrderCorrection {
	...
	protected function _getItems() {
		...
		$select
			->joinLeft(
				array('o' => $collection->getTable('sales_order')),
				'main_table.order_id = o.entity_id',
				array('o.increment_id', 'o.original_increment_id')
			)
			->joinLeft(
				array('cmo' => $collection->getTable('sales_creditmemo')),
			...
			->where(
				...
				'NOT (main_table.product_type="simple" AND main_table.parent_item_id IS NOT NULL AND main_table.row_total=0)'
			)
			->where(
